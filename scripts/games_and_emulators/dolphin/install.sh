...
case "$__os_id" in
    ...
    ;;
    Fedora)
        sudo dnf install -y vulkan-loader vulkan-loader-devel cmake git gcc-c++ libXext-devel libgudev qt5-qtbase-devel systemd-devel openal-soft-devel libevdev-devel libao-devel SOIL-devel libXrandr-devel pulseaudio-libs-devel bluez-libs-devel libusb-devel || error "Failed to install dependencies!"
    ;;
    *)
