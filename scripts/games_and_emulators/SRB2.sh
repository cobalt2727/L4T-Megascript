...
Raspbian | Debian | Ubuntu)
  ...
  ;;
Fedora)
  sudo dnf install -y --refresh wget cmake gcc gcc-c++ unzip git git-lfs SDL2-devel SDL2_mixer-devel libcurl-devel libopenmpt-devel game-music-emu-devel libpng-devel zlib-devel || error "Dependency installs failed"
  ;;
*)
...
bionic | focal)
  ...
  ./configure --prefix=/usr/local --with-ssl --with-nghttp2 LDFLAGS="-Wl,-rpath,/usr/local/lib"
  ...
  /usr/local/bin/curl --version || { sudo rm -f /usr/local/bin/curl; error "Failed to build curl properly!"; }
