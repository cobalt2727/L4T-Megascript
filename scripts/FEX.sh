...
#wipe LLVM 14 installs from previous setups
...
bionic | focal)
  ...
  if package_installed "libclang-14-dev"; then
    sudo apt remove libclang-14-dev -y
  fi
...
if package_installed "llvm-14"; then
  sudo apt remove llvm-14 clang-14 -y
fi
...
case "$__os_codename" in
bionic | focal)
  CC=clang-19 CXX=clang++-19 AR=llvm-ar-19 LINKER=lld-19 NM=llvm-nm-19 OBJDUMP=llvm-objdump-19 RANLIB=llvm-ranlib-19 cmake -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=Release -DENABLE_LTO=True -DCMAKE_C_FLAGS_INIT="-static" -DBUILD_TESTS=False -G Ninja .. || error "cmake failed!"
  CC=clang-19 CXX=clang++-19 AR=llvm-ar-19 LINKER=lld-19 NM=llvm-nm-19 OBJDUMP=llvm-objdump-19 RANLIB=llvm-ranlib-19 ninja || error "Failed to build!"
  ;;
*)
