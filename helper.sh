...
if grep -q debian /etc/os-release; then
  ...
  if [[ $gui == "gui" ]]; then
    pkexec sh -c "dnf install -y --refresh $(echo "${dependencies[@]}"); hash -r; repository_branch=$repository_branch; repository_username=$repository_username; hash -r"
  else
    sudo sh -c "dnf install -y --refresh $(echo "${dependencies[@]}"); hash -r; repository_branch=$repository_branch; repository_username=$repository_username; hash -r"
  fi
fi
